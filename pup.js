...
(async () => {
  ...
  const ordersArr = await page.evaluate(() => Array.from(document.querySelectorAll('.sc-1mo3ldo-0 div[width] .sc-1hez2tp-0')).map((partner) => partner.innerText));
  const name = await page.evaluate(() => Array.from(document.querySelectorAll('.sc-1hp8d8a-0')).map((partner) => partner.innerText));
  const linkData = await page.evaluate(() => Array.from(document.querySelectorAll('.sc-1mo3ldo-0 div.jumbo-tracker a[href]')).map((partner) => partner.getAttribute('href')));
  ...
  for(let i = 0; i < ordersArr.length; i++){
    ...
    if(newPush != detectCopy && isNaN(newPush) == false){
      newArr.push(newPush);
      detectCopy = newPush;
    }
  }
  ...
  for(let i = 13; i <=linkData.length; i++){
    ...
    if(baseUrl + linkData[i] != detectCopyURL){
